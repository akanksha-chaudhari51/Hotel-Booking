...
def book_room():
    ...
    if room_no in room and room[room_no]['status'] == 'available':
        room[room_no]['status'] = 'booked'
        booking.append({'guest': name, 'room': room_no})
        print(f"Room {room_no} Booked for {name}.")
        print(f"Total Price: Rs.{room[room_no]['price']}")
    else:
        print("Room Not Available or Invalid Room No.")
...
def cancel_booking():
    ...
    for b in booking:
        if b['guest'] == name:
            ...
            booking.remove(b)
            print(f"Booking Cancelled for {name} (Room {room_no}).")
            found = True
            break
    if not found:
